...
| 能力 | 说明 |
| --- | --- |
| **多格式支持** | TXT、Markdown、PDF、PNG、JPG/JPEG、Word（.doc/.docx）、Excel（.xlsx/.xls）、CSV |
| **Word 智能解析** | `.docx` 由 python-docx 提取段落与表格；`.doc` 优先经 LibreOffice 转换为 `.docx` 以保留表格等复杂结构，转换失败或未配置时自动降级 antiword 纯文本提取 |
| **表格智能分片** | Excel/CSV 转为"行N → 列名: 值"的行结构化文本，每个分片自带表头可独立检索；按工作表边界独立切片，metadata 记录 `_sheet_name`，避免跨表混切 |
...
| **Word 解析** | python-docx + LibreOffice + antiword | `.docx` 由 python-docx 提取；`.doc` 优先 LibreOffice 转换保留结构，降级 antiword 纯文本 |
| **Excel/CSV 解析** | openpyxl + xlrd + csv | `.xlsx` 用 openpyxl、`.xls` 用 xlrd、CSV 用标准库，统一转行结构化文本 |
...
不同格式按"保真优先、逐级降级"策略解析：

- **Word `.docx`**：python-docx 提取段落与表格，结构完整
- **Word `.doc`**：优先 LibreOffice headless 转换为 `.docx` 以保留表格等复杂结构；转换失败或未配置时降级 antiword 纯文本提取，保证可用性
- **Excel/CSV**：统一转为"行N → 列名: 值"的行结构化文本，每行自带表头，分片后仍可独立检索；按工作表边界切片，避免跨表混切
...
# ===== Word/Excel 解析配置 =====
# .doc 解析：优先 LibreOffice 转 .docx（保留表格结构），失败降级 antiword 纯文本
libreoffice_path=     # LibreOffice (soffice) 可执行文件路径，留空则跳过转换
antiword_path=        # antiword 可执行文件路径，LibreOffice 不可用时的降级方案
...
| `libreoffice_path` | - | LibreOffice (soffice) 路径，`.doc` 转 `.docx` 以保留表格等复杂结构 |
| `antiword_path` | - | antiword 路径，`.doc` 解析的降级纯文本方案 |
...
- **Word 文档解析**：支持 `.doc` / `.docx` 格式。`.docx` 由 python-docx 提取段落与表格；`.doc` 优先经 LibreOffice 转换为 `.docx` 以保留表格等复杂结构，转换失败或未配置时自动降级 antiword 纯文本提取
- **Excel/CSV 文件支持**：支持 `.xlsx` / `.xls` / `.csv` 格式。`.xlsx` 用 openpyxl、`.xls` 用 xlrd、CSV 用标准库读取，统一转为行结构化文本
- **表格智能分片**：Excel/CSV 按工作表边界独立切片，每个分片自带表头可独立检索，metadata 记录 `_sheet_name`
- **配置项**：新增 `antiword_path` 与 `libreoffice_path`，用于 `.doc` 文件的分级解析
- **前端**：文件上传组件支持 Word 与表格文件类型选择

#### 依赖

- 新增 `python-docx`、`openpyxl`、`xlrd`
